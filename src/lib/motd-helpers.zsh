...
# ==============================================================================
# Helper 6: Banner Theme System
# ==============================================================================

# Theme metadata: slug|label|description
FRANKLIN_MOTD_THEME_METADATA=(
...
# ==============================================================================
# Banner Renderers - Thick Theme (Current Default)
# ==============================================================================

_motd_render_banner_thick() {
    local hostname="$1"
    local ip_address="$2"
    local bg_sequence="$3"
    local fg_sequence="$4"
    ...
    local text_color_code="\033[38;2;247;248;249m"  # Off-white
    if [[ "$text_color" == "black" ]]; then
        text_color_code="\033[38;2;43;48;59m"  # Near-black
    fi
    local reset="${NC:-\033[0m}"

    # Top row: Full-width LOWER HALF BLOCK (▄) in foreground color
    ...
    # Middle row: Background fill + hostname/IP text
    ...
    local middle_row="${bg_main}${text_color_code}${label}${spaces}${reset}"

    # Bottom row: Full-width UPPER HALF BLOCK (▀) in foreground color
...
# ==============================================================================
# Banner Renderers - Thin Theme (Single-Line Box Drawing)
# ==============================================================================

_motd_render_banner_thin() {
    local hostname="$1"
    local ip_address="$2"
    local bg_sequence="$3"
    local fg_sequence="$4"
    ...
    # Middle row: │ + background fill + label + padding + │
    local label=" ${hostname} (${ip_address})"
    ...
    local middle_row="${fg_darker}│${bg_main}${text_color_code}${label}${spaces}${reset}${fg_darker}│${reset}"
...
# ==============================================================================
# Banner Renderers - Thin Double Theme (Double-Line Box Drawing)
# ==============================================================================

_motd_render_banner_thin_double() {
    local hostname="$1"
    local ip_address="$2"
    local bg_sequence="$3"
    local fg_sequence="$4"
...
# ==============================================================================
# Theme Dispatcher - Route to correct banner renderer
# ==============================================================================

_motd_render_banner_themed() {
    local hostname="$1"
    local ip_address="$2"
    local bg_sequence="$3"
    local fg_sequence="$4"
    ...
    case "$theme" in
        ...
        thin-double)
            ...
            ;;
        *)
            # Invalid theme, fall back to thick
            _motd_render_banner_thick "$@"
            ;;
    esac
}
