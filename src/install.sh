...
configure_motd_theme() {
  if [ "${FRANKLIN_TEST_MODE:-0}" -eq 1 ]; then
    log_debug "Skipping MOTD theme configuration (test mode)"
    return 0
  fi

  local config_dir="$FRANKLIN_CONFIG_DIR"
  local config_file="${config_dir}/motd.env"
  local selection="${FRANKLIN_MOTD_THEME:-}"
  local chosen_theme="thick"

  # CLI flag / env var takes precedence
  if [ -n "$selection" ]; then
    case "$selection" in
      thick|thin|thin-double)
        chosen_theme="$selection"
        ;;
      *)
        log_warning "Invalid --motd-theme value '$selection'; using default 'thick'"
        ;;
    esac
  # Interactive selection
  elif [ -t 0 ] || [ -r /dev/tty ]; then
    echo ""
    log_info "Configure Franklin banner style"

    # Theme metadata from motd-helpers.zsh: slug|label|description
    local -a theme_entries=(
    ...
    for entry in "${theme_entries[@]}"; do
      IFS='|' read -r slug label desc <<<"$entry"
      echo ""
      # Render sample banner preview
      franklin_render_theme_preview "$slug" "${MOTD_COLOR:-#4C627D}"
      printf "  %d) %-12s — %s\n" "$idx" "$label" "$desc"
      ((idx++))
    done

    echo ""
    printf "Select a banner theme [1]: "
    local menu_choice=""
    read -r menu_choice </dev/tty || menu_choice=""
    menu_choice="${menu_choice:-1}"

    if [[ $menu_choice =~ ^[0-9]+$ ]] && [ "$menu_choice" -ge 1 ] && [ "$menu_choice" -le "${#theme_entries[@]}" ]; then
      IFS='|' read -r chosen_theme _ _ <<<"${theme_entries[$((menu_choice - 1))]}"
    else
      log_warning "Invalid choice, using default 'thick'"
    fi
  else
    log_info "Skipping MOTD theme prompt (non-interactive). Re-run with --motd-theme <value> to configure."
  fi

  # Append to motd.env (preserve MOTD_COLOR if already present)
  if [ -f "$config_file" ]; then
    # Update existing theme line or append
    if grep -q "^export MOTD_THEME=" "$config_file"; then
      # Update in place (macOS compatible way without -i)
      local tmpfile
    ...
    # New file (should not happen if color picker ran first, but handle gracefully)
    mkdir -p "$config_dir"
    cat > "$config_file" <<EOF
# Generated by Franklin install.sh on $(date +%Y-%m-%dT%H:%M:%S%z)
...
EOF
  ...
  chmod 600 "$config_file" 2>/dev/null || true
  export MOTD_THEME="$chosen_theme"
  log_success "Saved MOTD banner theme ($chosen_theme) to $config_file"
}

franklin_render_theme_preview() {
  local theme="$1"
...
install_log_spacer
  ...
  begin_install_phase "MOTD banner theme"
  configure_motd_theme || true
